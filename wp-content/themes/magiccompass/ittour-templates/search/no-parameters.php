<?php
/**
 * Created by PhpStorm.
 * User: snth
 * Date: 20.09.18
 * Time: 17:24
 */
?>

<div id="search-result" class="search-result container">
    <div class="col-xs-12">
        <div class="search-result__container">
            <div class="search-result__inner">
                <h1 class="page-title"><?php echo __('Start Searching', 'snthwp'); ?></h1>
                <p>
                    Lorem ipsum dolor sit amet, his graecis offendit ne, in nec clita eloquentiam, ex sed sanctus placerat temporibus. Id pri viris quidam. Ea has cetero aperiri, in quot rationibus est, an dico vocibus mea. Eum virtute dolorem pertinacia ne, mei congue integre molestie no. Ut malis elitr vituperata pro, mel ex senserit dissentiet, nam alii nominati molestiae cu. Quot harum labitur mei id, cu ceteros contentiones eos.
                </p>
                <p>
                    Mel debet iisque numquam no, graeci molestie duo ut. Id eos purto inimicus disputationi. Maiorum feugait duo et, mei ex suas assum, nam verear indoctum laboramus ea. Discere fabulas no eum, ne agam recusabo suscipiantur vim, ius an insolens sensibus.
                </p>
                <p>
                    Mel aeque dolorem qualisque te, sit in veri munere. Nam solet accusam vituperatoribus no, vel impetus blandit percipitur an. Per harum eruditi appareat in, sed utinam intellegat eloquentiam ea. No pro vocibus erroribus adversarium, omittam officiis ius te.
                </p>
            </div>
        </div>
    </div>
</div>
